"""Generate the Calyps0 V68 NoPS2 forced-sweep diagnostic.

The original InitRDRAM implementation and settings remain unchanged. Its real
signed return is preserved and displayed. Every returned value runs the
Channel A/B sweep. Component probes run afterward only when InitRDRAM
succeeded and both measured channel results passed. The final screen is held.
"""
...
# Forced-sweep variant: every returned InitRDRAM value enters the sweep.
...
a.jump('screen',True)
# After Channel A, proceed to B. Once both channels finish, component probes
# run only if InitRDRAM succeeded and both measured mismatch counts are zero.
a.branch(4,30,0,'start_b')
a.branch(1,16,0,'tests_done')
a.branch(5,17,0,'tests_done')
a.branch(5,18,0,'tests_done')
a.jump('eegs_tests')
a.label('eegs_tests')
...
# status_clear contains its GIFtag plus ten A+D writes: RGBAQ, four complete
# sprite rectangles, and SIGNAL. Send all eleven qwords so both Channel A and
# Channel B result/rate fields are erased before their replacements are drawn.
a.addr(4,'status_clear');a.i(9,5,0,11);a.jump('send_rom',True)
a.addr(4,'white');a.i(9,5,0,2);a.jump('send_rom',True)
...
# Forced-sweep variant: always show progress followed by measured results.
...
# Five passes each compare 2**22 words per channel. Format
# round(bad * 10000 / (5 * 2**22)) as 0.00%..100.00%. Cancelling the factor
# of five gives bad * 2000 / 2**22, which keeps the calculation shift-based
# and avoids unsupported DDIVU/DMULTU.
a.label('percent');a.i(9,29,29,-16);a.i(0x3f,31,29,0);a.move(8,0)
for shift in [10,9,8,7,6,4]:a.r(0x38,9,0,4,shift);a.r(0x2d,8,8,9)
a.li(9,0x00200000);a.r(0x2d,8,8,9);a.r(0x3a,8,0,8,22);a.i(9,12,0,100);a.r(0x1b,0,8,12);a.r(0x12,9);a.r(0x10,10)
...
def main():
    ...
    name='Calyps0_V68_NoPS2_Forced_Sweep_Fixed'
    entry=a.labels['real']
    ...
    manifest={'base_sha256':BASE_SHA,'output_sha256':hashlib.sha256(buf).hexdigest(),
              ...
              'notes':[
                'Calyps0 V68 NoPS2 forced-sweep diagnostic',
                'Original InitRDRAM implementation and settings remain unchanged',
                'Actual signed InitRDRAM return is preserved and displayed',
                'Every returned InitRDRAM value runs the sequential Channel A/B RDRAM diagnostic',
                'Component probes execute only after successful initialization and zero measured mismatches on both channels',
                'Both channel rows display TEST IN PROGRESS until measured results are ready',
