"""Build the Calyps0 V68 NoPS2 diagnostic from verified inputs.

Chain: original ps2_emu.elf -> verified TESTMODE display base -> V68
(conditional RDRAM sweep plus component-test screen).
"""
...
OUTPUT_SHA = "2f8c17586b977b1eb0752c8a89abca7985c8f459e54569614a2bf76485d0a581"

OSDSYS_OFFSET = 0x2EC818
OSDSYS_SLOT_SIZE = 0x3DEF8
OUTPUT_NAME = "Calyps0_V68_NoPS2_Test.elf"

# Instruction replacements required by the verified TESTMODE display base.
# Each tuple is
...
def make_display_base(build_a: bytes) -> bytes:
    ...
    for offset, old_word, new_word in DISPLAY_BASE_CHANGES:
        ...
        if result[offset:offset + 4] != old:
            raise SystemExit(f"Display-base prerequisite mismatch at file offset {offset:#x}")
...
def main() -> None:
    if len(sys.argv) not in (3, 4):
        raise SystemExit(
            "Usage: py build_v68.py original_ps2_emu.elf TEST_ROM0 [output_directory]"
        )
    ...
    display_base = make_display_base(build_a)
    ...
    with TemporaryDirectory() as temporary:
        ...
        subprocess.run(
            [sys.executable, str(root / "v68_payload.py"), str(display_base_path), str(generated_path)],
            check=True,
        )
        result = (generated_path / OUTPUT_NAME).read_bytes()

    require_hash("final V68 output", result, OUTPUT_SHA)
